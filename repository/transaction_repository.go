...
func (r *transactionRepository) InsertTransaction(trx entity.Transaction) (entity.Transaction, error) {
	err := r.db.Raw("INSERT INTO transaction (participant_id, creator_id, event_id, amount, status_payment, created_at) VALUES (@ParticipantId, @CreatorId, @EventId, @Amount, @StatusPayment, @CreatedAt)", trx).Save(&trx).Error
	if err != nil {
		return trx, err
	}
	return trx, nil
